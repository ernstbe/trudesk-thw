...
const SettingItem = ({ title, subtitle, component, tooltip, extraClass, children, subPanelPadding, footer }) => {
  return (
      ...
      <div className={'panel trupanel nopadding no-hover-shadow ' + extraClass || ''} style={{ minHeight: '60px' }}>
        ...
        {children && (
          <div>
            ...
            <div className='panel-body2' style={{ padding: subPanelPadding || '20px 15px 15px 15px' }}>
        ...
        {footer && (
          <div style={{ padding: '0 15px 25px 15px', marginTop: '-15px' }}>{footer}</div>
        )}
      </div>
    </div>
  )

  ...
  render () {
    ...
    const items =
      ...
      this.props.accountsState.accounts.map(user => {
        ...
        let actionMenu = [<DropdownItem key={0} text={t('common.edit')} onClick={e => this.onEditAccountClicked(e, user)} />]
        if (user.get('deleted'))
          actionMenu.push(<DropdownItem key={2} text={t('accounts.enableAccount')} onClick={e => this.onEnableAccountClicked(e, user)} />)
        else
          actionMenu.push(
            <DropdownItem
              key={1}

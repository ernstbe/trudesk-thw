...
function AccountsImportContainer () {
  const csvRef = useRef(null)
  const jsonRef = useRef(null)
  const ldapRef = useRef(null)

  const csvWizardRef = useRef(null)
  const jsonWizardRef = useRef(null)
  const ldapWizardRef = useRef(null)

  const selectAccountImport = useCallback((event, type) => {
    if (!csvRef.current || !jsonRef.current || !ldapRef.current) return
    if (!csvWizardRef.current || !jsonWizardRef.current || !ldapWizardRef.current) return
    if (event.target.classList.contains('card-disabled')) return

    switch (type) {
      case 'csv':
        // csvWizardRef.current.classList.remove('uk-hidden')
        csvWizardRef.current.show()
        jsonRef.current.classList.add('card-disabled')
        ldapRef.current.classList.add('card-disabled')
        break
      case 'json':
        jsonWizardRef.current.classList.remove('uk-hidden')
        csvRef.current.classList.add('card-disabled')
        ldapRef.current.classList.add('card-disabled')
        break
      case 'ldap':
        ldapWizardRef.current.classList.remove('uk-hidden')
        csvRef.current.classList.add('card-disabled')
        jsonRef.current.classList.add('card-disabled')
        break
    }
  ...
  const resetWizards = useCallback(() => {
    if (!csvRef.current || !jsonRef.current || !ldapRef.current) return
    if (!csvWizardRef.current || !jsonWizardRef.current || !ldapWizardRef.current) return

    csvRef.current.classList.remove('card-disabled')
    jsonRef.current.classList.remove('card-disabled')
    ldapRef.current.classList.remove('card-disabled')

    csvWizardRef.current.hide()
    jsonWizardRef.current.classList.add('uk-hidden')
    ldapWizardRef.current.classList.add('uk-hidden')
  }, [])
  ...
  return (
    <>
      ...
      <PageContent>
        <div className='uk-grid uk-grid-medium uk-margin-medium-bottom js-wizard-select-wrapper'>
          ...
          <div className='uk-width-1-3'>
            <div
              ...
              style={{ minHeight: 85 }}
              onClick={e => selectAccountImport(e, 'csv')}
            >
          ...
          <div className='uk-width-1-3'>
            <div
              ...
              style={{ minHeight: 85 }}
              onClick={e => selectAccountImport(e, 'json')}
            >
              <div className='tru-card-content'>
                <div className='right uk-margin-small-top'>
          ...
          <div className='uk-width-1-3'>
            <div
              ...
              style={{ minHeight: 85 }}
              onClick={e => selectAccountImport(e, 'ldap')}
            >
              <div className='tru-card-content'>
                <div className='right uk-margin-small-top'>
        ...
        />
        <div id='json_wizard_card' ref={jsonWizardRef} className='uk-grid uk-margin-small-bottom uk-hidden'>
          <div className='uk-width-1-1'>
            ...
            >
              ...
              <div className='right' style={{ margin: 15 }}>
                <button className='btn md-btn md-btn-warning js-wizard-cancel' onClick={resetWizards}>
                  Cancel
                </button>
              </div>
        ...
        <div id='ldap_wizard_card' ref={ldapWizardRef} className='uk-grid uk-margin-small-bottom uk-hidden'>
          <div className='uk-width-1-1'>
            <div
              className='panel trupanel nopadding no-hover-shadow'
              style={{ position: 'relative', minHeight: 265 }}

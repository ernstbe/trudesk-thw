...
const EditPriorityPartial = ({ priority, fetchSettings }) => {
  const editPriorityRef = useRef(null)

  const toggleEditPriority = useCallback(() => {
    const $parent = $(editPriorityRef.current).parent()
  ...
  const onSubmitEditPriority = useCallback(
    (e, priority) => {
      e.preventDefault()
    ...
    [toggleEditPriority, fetchSettings]
  )

  return (
    <div className='edit-priority hide' style={{ paddingTop: '2px' }} ref={editPriorityRef}>
      <form onSubmit={e => onSubmitEditPriority(e, priority)}>
